...
def replace_null_values(**kwargs):
    df = pd.read_csv("/home/keruiiia/airflow/data/tiktok_google_play_reviews.csv")
...
def sort_by_created_date(**kwargs):
    ...
    output_file_path = "/home/keruiiia/airflow/data/sorted_tiktok_without_nulls.csv"
    df.to_csv(output_file_path, index=False)
    kwargs['ti'].xcom_push(key='output_file_path', value=output_file_path)


def clean_content_column(**kwargs):
    input_file_path = kwargs['ti'].xcom_pull(key='output_file_path', task_ids='sort_by_created_date_task')
    df = pd.read_csv(input_file_path)
    pattern = r"[^a-zA-Z\s\.\,\?\!']"
    df['content'] = df['content'].apply(lambda x: re.sub(pattern, '', x))
    output_file_path = "/home/keruiiia/airflow/data/sorted_tiktok_without_nulls_and_emojis.csv"
    df.to_csv(output_file_path, index=False)
    kwargs['ti'].xcom_push(key='output_file_path', value=output_file_path)


def upload_to_mongodb(**kwargs):
    input_file_path = kwargs['ti'].xcom_pull(key='output_file_path', task_ids='clean_content_column_task')
...
with DAG('tiktok_processing_3',
    ...
    def data_processing_tasks():
        replace_null_values_task = PythonOperator(
            task_id='replace_null_values_task',
            python_callable=replace_null_values,
            provide_context=True
        )

        sort_by_created_date_task = PythonOperator(
            task_id='sort_by_created_date_task',
            python_callable=sort_by_created_date,
            provide_context=True
        )

        clean_content_column_task = PythonOperator(
            task_id='clean_content_column_task',
